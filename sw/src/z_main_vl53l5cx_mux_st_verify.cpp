...
extern "C"
{
#include "sensors/vl53l5cx_uld_verify/vl53l5cx_api.h"
}
...
int32_t verify_get_tick()
{
    return (int32_t)millis();
}

int32_t verify_write(uint16_t address8, uint16_t reg, uint8_t *data, uint16_t length)
{
...
int32_t verify_read(uint16_t address8, uint16_t reg, uint8_t *data, uint16_t length)
{
    const uint8_t address7 = (uint8_t)(address8 >> 1);
    uint16_t offset = 0;
...
{
    uint8_t buffer[2] = {0};
    if (verify_read((uint16_t)(VL53L5CX_ADDR << 1), 0x010F, buffer, 2) != 0)
    {
        return false;
    }
...
bool initRanging()
{
    ...
    g_config.platform.Write = verify_write;
    g_config.platform.Read = verify_read;
    g_config.platform.GetTick = verify_get_tick;
...
void setup()
{
    ...
    delay(500);
    Serial.println();
    Serial.println(F("VL53L5CX ST verify mux test"));
    Serial.println(F("No drivetrain modules are included in this env."));
    Serial.println(F("This env uses a separate pristine ST init copy over raw Wire."));
