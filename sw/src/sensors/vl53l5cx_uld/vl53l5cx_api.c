...
#include "sensors/vl53l5cx_uld/vl53l5cx_buffers.h"
...
static uint8_t g_vl53l5cx_debug_init_step = 0;

uint8_t vl53l5cx_debug_init_step(void)
{
	return g_vl53l5cx_debug_init_step;
}
		...
		VL53L5CX_Configuration		*p_dev)
{
	...
	g_vl53l5cx_debug_init_step = 2;
	status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);
	tmp = _vl53l5cx_poll_for_answer(p_dev, 1, 0, 0x06, 0xff, 1);
	if(tmp != (uint8_t)0)
	{
		/*
		 * On this project hardware path, the early boot-ready poll can time out
		 * even though the sensor still answers to later direct register reads.
		 * Keep going into the firmware access path so we can determine whether
		 * the device becomes usable once the ranging core is loaded.
		 */
		status |= 0;
	}
