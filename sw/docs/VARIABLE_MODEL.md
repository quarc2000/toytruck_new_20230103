...
| 9010 | `configExpanderPresent` | config | Whether the runtime detected the TCA9548 mux or expander path at startup | boolean-like integer | `0` absent, `1` present | active runtime startup and device services | Active | This is a runtime hardware-detection flag, not a truck-ID declaration. Optional expander-backed services should degrade cleanly when this is `0`. |
| 9011 | `configGy271Present` | config | Whether the runtime detected and started the GY-271 magnetometer path | boolean-like integer | `0` absent or failed, `1` present | `src/sensors/gy271_service.cpp` | Active | This lets the runtime and diagnostics distinguish gyro-only fallback from a healthy magnetic-reference path. |
| 9012 | `configFrontLidarPresent` | config | Whether the runtime detected at least one working front VL53L0X sensor on the expander path | boolean-like integer | `0` absent, `1` present | `src/sensors/front_vl53l0x_service.cpp` | Active | This is intentionally runtime-detected so the same firmware can run on trucks with or without the front lidar pair. |
| 9013 | `configMagHeadingValid` | config | Whether the runtime has received at least one real magnetic course sample from the GY-271 | boolean-like integer | `0` not yet, `1` yes | `src/sensors/gy271_service.cpp` | Active | This distinguishes "magnetometer path exists" from "a real absolute heading sample has actually been published". |
| 9014 | `configHeadingReady` | config | Whether the runtime has accepted a trustworthy startup heading reference | boolean-like integer | `0` not ready, `1` ready | `src/fusion/fusion_service.cpp` | Active | When the GY-271 is present, this now flips only after `configMagHeadingValid` is set and the fused heading has been initialized from the magnetic course. The explorer waits for this before starting. |
...
| 4010 | `fuseHeadingDeg10` | Best current heading estimate after combining gyro and magnetic reference | tenths of degrees | `src/fusion/fusion_service.cpp` | Active, Uncertain | Current slow fusion follows gyro heading deltas for short-term responsiveness and corrects back toward `calculatedMagCourse` when the GY-271 disturbance flag is clear. This is now the runtime-facing heading for `env:exploremap`, with `0 = magnetic north`. |
...
| `fuseHeadingDeg10` | `fused*` | This is now the best current heading after combining gyro integration with magnetic reference, and should later also absorb map-based correction. |
...
| slow fusion task | `100 ms` today | `calcHeading`, `calculatedMagCourse`, `calculatedMagDisturbance`, later map alignment and wheel cues | `fuseHeadingDeg10`, later other pose and navigation-facing `fused*` outputs |
