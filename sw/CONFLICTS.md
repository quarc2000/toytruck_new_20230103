...
| CON-0002 | Shared state and concurrent function access must be task safe | `.context/GUARDRAILS.md` | architecture_conflict | FAIL | Resolved | `src/sensors/usensor.cpp` used shared file-scope state across the trigger task, the echo ISR, and sensor registration without an approved synchronization or serialization model, so task-safe behavior was not assured. | User directed remediation. The module now serializes shared state with a `portMUX`, no longer calls `globalVar_set()` from the ISR, and starts the polling task only after the first sensor is registered. | patbwm | 2026-03-13 | `sensor[]`, `num_sensors`, `current_sensor`, pulse state, and pending measurement state are now protected by an interrupt-safe critical section. Measurement publication was moved out of the ISR into the task context. Build verification is still pending because local PlatformIO runs are blocked by `C:\Users\patbwm\.platformio\platforms.lock`. | Review and remediate `usensor.cpp` shared-state model |  |
